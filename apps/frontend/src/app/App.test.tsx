...
// B2a Task 7：壳页已被屏5 真页替换，断言随之改为新页首屏（标题 + 空态文案），
// **不删除本用例**——它守的是「/admin/gaps 这条路由确实挂着问题池页」。
it("renders GapsPage on /admin/gaps (B2a 屏5 问题池)", async () => {
  localStorage.setItem("token", "fake-token");
  render(
  ...
  expect(await screen.findByText("知识缺口 / 问题池")).toBeInTheDocument();
  expect(
