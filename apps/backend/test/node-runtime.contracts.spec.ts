import { INTENT_OUTPUT_KEYS, INTENT_TABLE, NODE_CONTRACTS } from "@codecrush/contracts";
...
describe("INTENT_CONTRACT（014 D3：enum 静态闭集 + availableIntents 全表）", () => {
  it("fallback：intent=UNKNOWN，confidence=0（无 routeIds）", () => {
  ...
  it("extraValidate 已删除（enum 收口后不再需要动态值域校验）", () => {
    expect(INTENT_CONTRACT.extraValidate).toBeUndefined();
  });
  it("outputSchema 接受 INTENT_OUTPUT_KEYS 闭集内的每一个 key", () => {
    for (const key of INTENT_OUTPUT_KEYS) {
      expect(INTENT_CONTRACT.outputSchema.safeParse({ intent: key, confidence: 0.5 }).success).toBe(
        true,
      );
    }
  });
  it("outputSchema 在 schema 解析层拒绝闭集外的 intent（非事后校验）", () => {
    for (const bad of ["其它", "售后", "unknown", "chat"]) {
      expect(
        INTENT_CONTRACT.outputSchema.safeParse({ intent: bad, confidence: 0.5 }).success,
      ).toBe(false);
    }
  });
  it("outputSchema 拒绝 confidence 超出 0-1 范围", () => {
    expect(
      INTENT_CONTRACT.outputSchema.safeParse({ intent: "SUPPORT", confidence: 1.5 }).success,
    ).toBe(false);
  });
  it("reservedDataSchema 要求 availableIntents 为 {key,label,criteria} 对象数组", () => {
    expect(
      INTENT_CONTRACT.reservedDataSchema.safeParse({ availableIntents: INTENT_TABLE }).success,
    ).toBe(true);
    expect(INTENT_CONTRACT.reservedDataSchema.safeParse({}).success).toBe(false);
    expect(
      INTENT_CONTRACT.reservedDataSchema.safeParse({ availableIntents: ["kb_a"] }).success,
    ).toBe(false);
  });
});
