...
// 012 重构：版本平权 + 排他标签。0011 为加法迁移（新列可空/带默认，旧 status/current_version_id
// 仍在，兼容窗口内 insert 显式写 status='draft'）；0012（Story 4）删旧列并收紧非空。

export const prompts = pgTable("prompts", {
  id: uuid("id").primaryKey().defaultRandom(),
  name: text("name").notNull().unique(),
  node: text("node").notNull(),
  // 012 已废弃「当前生产版本」指针；兼容窗口保留列，Story 4 清理迁移删除
...
export const promptVersions = pgTable(
  "prompt_versions",
  {
    ...
    // 012 新增：保存时服务端跑 compilePromptBody() 的结果。加法迁移期可空，
    // backfill 补齐后由 Story 4 清理迁移收紧 NOT NULL。
    compileStatus: text("compile_status"),
    compileErrors: jsonb("compile_errors").$type<CompileIssue[]>(),
    note: text("note"),
    author: text("author").notNull(),
    // 012 已废弃三态状态机；兼容窗口保留列（NOT NULL DEFAULT 'draft'），Story 4 删除
    status: text("status").notNull().default("draft"),
    createdAt: timestamp("created_at").notNull().defaultNow(),
  },
  (t) => ({
    // D8：unique(promptId, version) 兜底并发撞号；index(promptId, status) 加速 prod 查询（Story 4 随 status 删）
