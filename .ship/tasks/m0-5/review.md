...
### P3: `POST /traces/hello` 在 tracing 禁用时返回全零假身份
...
### P3: `GET /traces/:traceId` 无入参校验，畸形 id 返回违反契约的 200
...
### P3: 冷库上 `GET /traces/:id` 会阻塞 10s 后 500
...
无 P1/P2。三个 P3 都是边缘路径的真实小 bug，不阻塞 M0.5 交付；建议随 M2（校验管道）/M9（trace UI）
两波顺手收掉，或现在用 10 分钟一起修。
