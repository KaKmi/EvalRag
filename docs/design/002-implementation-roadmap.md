...
>
> 🔺 **缺口 23 已提级为 E-W4 之后的第一优先（2026-07-21，B2b 上线后真环境实测）**：它不再只是「影响信号质量」，而是**卡死了 B2b 的核心闭环**。实测该环境下 `context_precision` 恒为 0，而回验分数取三项最小值 ⇒ 补库无论多成功都拿不到 80 ⇒ 簇被退回并打上**假阳性的「复发」标**（用户实测：检索命中、回答正确，系统却报「补库后仍低分」）。详见 [`021` §11.7](021-problem-pool-gap-domain.md)。
