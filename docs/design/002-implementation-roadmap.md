...
| **B2b（待做）** | **接手清单见 [`021` §12](021-problem-pool-gap-domain.md)**（B2a 交付后写的实测状态 + 已知取舍 + 待办，含理由与修法）。摘要：**[补知识库] 三步向导**（LLM 草拟 → 人审编辑 → 入库走现有管线，红线「无人审不入库」）+ **入库后自动回验**（重放代表问题 + 重新评分，≥80 转「已回验✓」显示 41→89）+ **缺口状态机四态**（需 `ALTER` `gap_clusters_status_check`）+ 「复发」角标 + 屏3「标记忽略」（**需先给 `eval_run_results` 加字段**）+ 入集重复检测的语义近似半（>0.95 + 强制加入）。**外加三项 B2a 降级收口**：`promote` 原子事务化、质心跨事务并发校验、以及**合并后优先做一次收集器生产观测**（决策 G 只在埋点后的新流量上验证过，历史 trace 无 `rag.rewrite.query`——见 021 §12 ①）。 | ⬜ 待做 |
