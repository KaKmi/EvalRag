...
**当前下一步**：**E-W4 已完结**（B2a + B2b 均已交付并合并；两条飞轮臂已在真实数据上完整闭环，见 [`021` §11.7](021-problem-pool-gap-domain.md)）。下一波待用户指定——**接手前必读 [`021` §12](021-problem-pool-gap-domain.md)**（B2b 交付后仍欠的账，其中「原型逐屏视觉比对从未完成」是唯一一条明知该做而完全没做的）。
...
| **B2b** | 问题池剩余：**[补知识库] 三步向导**（LLM 草拟 → 人审编辑 → 入库走现有管线，红线「无人审不入库」）+ **入库后自动回验**（重放代表问题 + 重新评分，≥80 转「已回验✓」并显示 41→89）+ **缺口状态机四态**（`drafting/reviewing/filled/verified`，需 `ALTER` `gap_clusters_status_check`）+ 「复发」角标 + 屏3 行尾「标记忽略」（**需要先给 `eval_run_results` 加可落该标记的字段**——B2a 明令不改 eval-runs schema，故未做）+ 入集重复检测的完整版（B2a 降级为「本批内部去重」，未与目标集既有用例做 >0.95 相似度比对） | **已交付并合并**（2026-07-21，[PR #47](https://github.com/KaKmi/RAGForge/pull/47)）。决策 I–N 见 [`021` §9b](021-problem-pool-gap-domain.md)；迁移 **0028**（CHECK 扩七态 + `gap_clusters` 9 新列 + `eval_run_results.ignored_at`）/ **0029**（`terminal_at`）。**红线守住**：LLM 草拟内容必须过 `reviewing` 人审节点才能入库——服务端在 upload **之前**硬拦状态，前端另有独立一道（勾「已核对」才可提交）。**每 story 独立 peer review**（完整对抗档），抓到并已修的要害：① 我自己在 `submitFill` 里把 upload 写在了状态检查**之前**，等于绕过本波唯一的红线（被自己的测试抓住）；② 回验的 `failed` 分支**生产不可达**——`notifyContentReplaced` 只在 `ready` 广播，补库文档解析失败会让簇永久卡在 `filled`（该态只剩「忽略」可走），故在平台层新开「文档终态」通道；③ 复发窗口按滚动 7 天算 ⇒ 刚被忽略的热门簇会被它**忽略之前**的历史立刻重开，「忽略」变成无效操作（迁移 0029 补 `terminal_at` 锚点）；④ 前端向导单实例常驻、state 跨簇存活 ⇒ 「簇 A 的问答被提交进簇 B」两条路径（load 失败残留 / 陈旧响应覆盖），复审用探针实测复现；⑤ 三轮复审共抓出**两条空测**（其中一条我自己做过变异检查却漏了，另一条的守卫写成比闭包 `clusterId`、恒成立等于没写——正是被那条空测「验证」通过的）。测试：`test:db` **14 suites/166**、后端 **1277**、前端 **350**、lint 0、tsc 0。**B2b 之后仍欠什么**：⇒ **完整接手清单见 [`021` §12](021-problem-pool-gap-domain.md)**（原型视觉比对从未做 / 回验缺可检索性守卫 / 聚类阈值 0.79 vs 0.85 / promote 重复 embedding / 验证覆盖空白）。另有全量问题聚类（M10 看板「热门问题」，见 `021` §11.6）与「仅重跑回验、不重新入库」的路径（`021` §11.8）。 |
