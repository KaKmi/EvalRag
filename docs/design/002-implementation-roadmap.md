...
| **M11** | 评测集 / 管理 / 报告 | 🔄 | 原列「里程碑 2 不做」，实际已按**评测飞轮 E-W 波次**推进：**E-W1 在线质量**（PR #31）+ **E-W2a 离线 run/评测集**（[PR #32](https://github.com/KaKmi/RAGForge/pull/32)，含运行时 QA 修复）+ **E-W2b-0 worker 拆进程**（[PR #35](https://github.com/KaKmi/RAGForge/pull/35)，019/018 缺口 19）+ **Judge scoring integrity v2**（020，迁移 `0021_judge_scoring_v2`；**2026-07-18 第二轮修复+真实验证已交付**——首版 `.max(100)` 假设被真实调用证伪，改为归一化+修复式重试后 32 条真实 trace 裁判失败数 0/32）已交付；**E-W2b**（重放/对比屏4/检索层指标）待做；~~E-W2c 思考 token 治理~~ **已由换模型解决、波次取消**（2026-07-17 实测）——见附录 A「评测飞轮（E-W）分波交付进度」 | 017 / 018 / 019 / 020 |
