...
| **E-W2b** | 重放、版本对比屏4、检索层 gold-docs 指标（Recall/NDCG/命中率）、Citation、每题重复聚合、配置版本引用保护、`timeoutMs` 硬中断（需 plumb AbortSignal）、doc→chunk 级 gold 选择器 | **已实现待 PR**（分支 `ship/e-w2b-4-gold-docs-recall-ndcg-citation-timeoutms-plumb-abort`）。交付：F1 AbortSignal 一路 plumb + `runForEvaluation` 硬中断（收口 018 缺口 9，超时成真墙钟）；F2 检索层三指标（`retrieval-gold-metrics.ts` 纯函数 + 编排带出 `retrievedHits` + worker 回填，NULL 不退化 0）；F3 chunk 级 `gold_doc_refs`（迁移 0022 回填 + 选择器）；F4 CitationEvaluator（仅记分卡，不进 verdict/综合分，`offline-v2` 不升号）；F5 `repeatCount` 双层循环 + 非空均值聚合（屏3/屏4 同源 `aggregateCaseRows`）；F6 应用删除守卫（收口缺口 5，注册式）；F7 单条重放 SSE + 即时判分（不落库/不发 span）；F8 屏4 版本对比（`GET /eval/runs/compare`）。**核心不变量守住**：离线/重放分数只落 PG 绝不发 `rag.eval` span、`OVERALL_SCORE`/`lastRunScore` 两段 SQL 零改动、线上 chat 路径逐字节不变（AC1-2）。技术债缺口 13/14/15/18 仍按用户指示后修。PR 链接由 handoff 补 |
