...
description: "自研轻量 Langfuse：Session>Trace>Observation 模型、OTLP 结束即写、大 payload offload、VIEW 读模型、两级 UI；本版聚焦 RAG 节点。"
category: "design"
number: "004"
status: draft
...
**读侧全是对 `otel_traces` 的 VIEW 投影**（不落第二套表）：
```
...
| 读模型 | `otel_traces` + 自有 VIEW 投影 Session/Trace/Obs | 自建 traces/observations 写表 | 最少写链路、纯 OTLP；投影逻辑集中在 VIEW，够用再演进到独立表 |
