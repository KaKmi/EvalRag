...
5. **rewrite 结果读一等属性 `rag.rewrite.query`**（~~初稿：靠解 `codecrush.io.output` 的 JSON~~ —— 那条路是死的，见 §6.2 的 B2a 订正）。
   本条的 Revisit **已在 B2a 实现时触发并落地**：chat 编排的 rewrite 节点补发 `rag.rewrite.query`，读侧直接取。
   遗留影响见下方 **§12 ①**（埋点前的历史 trace 无此属性 ⇒ 收集器对历史多轮问题一律判「指代未消解」）。
...
## 12. B2a 交付后：实测状态、已知取舍与 B2b 待办

> B2a 全波交付（2026-07-19，[PR #46](https://github.com/KaKmi/RAGForge/pull/46)）。
> 每 story 独立 peer review + 一次真实环境运行时 QA（手动路径端到端跑通）。
> 本节是 **B2b 会话的接手清单**——`.ship/` 被 gitignore，只有已提交的本文档跨会话可见。

### ① 收集器在真实流量上的行为**未端到端验证**（合并后优先观测）

QA 只覆盖了**手动入池**路径（它恒 `rewrite_resolved=false`）。**自动收集器**的决策 G——靠 rewrite 节点发的
一等属性 `rag.rewrite.query`（§6.2）——从没在「一条真·改写消解成功的新 trace」上端到端走通过。
叠加两件事使它更需要现场盯：

- **埋点是 B2a（e62b865）才加的**，此前的历史 trace **全都没有** `rag.rewrite.query` ⇒ 收集器对所有历史多轮问题
  一律判「指代未消解」。只有埋点之后的**新流量**才会正常消解。
- cron 是 `*/30`，QA 窗口内没触发。`test/gap-pool-isolation.spec.ts` 在真 PG+CH 上覆盖了「不污染屏1 读模型」，
  但**没有**覆盖「收集器真的聚出合理的簇」。

**行动**：合并上线后让收集器在生产跑几天，核对屏5 是否堆出像样的缺口簇、指代追问簇是否被正确判 `retrieval`。

### ② B2a 内部的三处主动降级（危害已被压掉，但 B2b 该收口）

- **`promote` 无原子事务** —— 逐条 `createCase`，中途失败会半批入集。危害靠两件事压掉：与目标集既有用例的
  **归一化精确去重**（让重试安全）+ 失败时把已建 `caseIds` 随 `PartialPromoteError` 抛回。
  **B2b 收口**：`EvalSetsService` 开一个可传 tx 的批量接口，把整批 + `markEnteredEvalSet` 包进一个事务。
- **质心跨事务丢失更新** —— 单轮跑超租约 `GAP_COLLECT_LEASE_MS`（10min）时，被接管的两个实例会并发
  read-modify-write 质心。损害是**聚类质量退化（质心偏一个成员的量级），不是丢数/不是错数**，与幂等键无关。
  **更便宜的修法**（不必把增量平均挪进 SQL）：把 processor 观察到的 `freq` 放进簇 UPDATE 的 `WHERE`
  做乐观并发校验，影响 0 行就抛哨兵回滚、对这一条重跑 `ingest`——判定仍 100% 在 `gap-clustering.ts`，
  且复用 §已建好的哨兵回滚通道。
- **入集重复检测只做了精确比对** —— embedding 相似度 **>0.95** 的「疑似重复，与用例 #N 相似」标记 +「仍强制加入」
  交互（原型 `:269`）留 B2b。降级理由：跨集语义比对要 embed 整个目标集、成本随集大小线性增长，
  且没有强制加入回路时它会退化成硬拒绝——比不做更糟。

> 另有一条**已覆盖但值得知道**：`GapsRepository.attachItem` 的「并发缝隙抛 `GapItemConflictError` → 回滚 →
> 外层回查」分支需两事务真并发才走得到，单进程 + 租约下不可达，**无自动化覆盖**。改那段代码请靠人工推演。

### ③ B2b 整波（用户 §0 早已拍板留的，不是遗漏）

`[补知识库]` 三步向导（LLM 草拟 → 人审编辑 → 走现有上传/切片/embedding 管线，红线「无人审不入库」）+
**入库后自动回验**（重放代表问题 + 重新评分，≥80 转「已回验✓」并显示 41→89）+
**缺口状态机四态**（`drafting/reviewing/filled/verified`，需 `ALTER` `gap_clusters_status_check`）+
「复发」角标 + 屏3 行尾「标记忽略」（**需先给 `eval_run_results` 加可落该标记的字段**——B2a 明令不改 eval-runs schema，故未做）。

### ④ 两处 cosmetic / 既有偏离（低优先）

- 屏5 `?status=ignored` 筛选下无结果时，空态复用了全局文案「问题池为空 — 低质量问答会自动聚类出现在这里」，语境上略不贴切。
- Trace 详情顶部操作组第五个按钮，原型写「复制 OTLP」，实现是「{ } 复制 JSON」——**M9 的既有偏离，非本波引入**，已在 `TraceDetailPage.test.tsx` 的顺序用例里留档。

### ⑤ 一条无害的操作残留

QA 期间收集器 cron 跑过，`gap_watermarks` 里留了一行游标状态。那是 worker 自己的运营状态、不是用户数据——留着无害（删了下轮会重新播种）。
