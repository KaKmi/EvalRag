...
8. **补库向导的前端组件是单实例常驻，state 跨簇存活**（B2b 第三轮 peer review 抓出，已修）。`GapsPage` 靠切 `open`/`clusterId` 复用同一个 `GapFillWizard` 实例（不是卸载重建），因此**任何**未在打开时显式清理的 state 都会带到下一个簇。已实测复现过两条「簇 A 的问答被提交进簇 B」的路径：① 打开时不清内容 + 新簇草稿拉取失败 ⇒ 屏上仍是 A 的内容且可提交；② 慢的 A 响应在切到 B 之后落地 ⇒ 覆盖 B。
   现有三道防线（打开时清空 / `load` 丢弃陈旧响应 / 渲染前校验 `draft.clusterId === clusterId`）**互为冗余**，任意一道单独存在都够用，回归测试钉的是「不得同时失去」。
