...
7. ⛔ **回验的结论可能与可观测事实相反，且低分时无法归因**（B2b 上线后**真环境实测**发现，2026-07-21）。
   ...
   **根因**（已查证）：
   - `verified_score = min(忠实度, 答案相关性, 上下文精确率)`（决策 K 的口径），
     三项明细**一项都不落库**；
   - 实测 ClickHouse 最近 8 条 trace 的 `context_precision` **无一例外是 0**
     （`faithfulness` 多为未评）。0 不是「检索得差」，是判分输入就是错的——
     成员都挂「指代未消解」标签，即 `018` 记录的**缺口 23**；
   - 三取最小 ⇒ 只要 `ctxprec` 被判 0，**答案再好也拿不到 80**。

   **后果**：「复发」标成了**假阳性**——它会驱使运营去查一个不存在的问题，
   甚至重补一遍同样的内容（下一次会因同一缺陷再次低分）。
   **一个会说谎的信号比没有信号更糟**，这直接侵蚀整个飞轮的可信度。
   也解释了 `≥80 → 已回验✓` 为何从未跑通：QA 判 72、用户判 62，
   **同一个根因**，不是「补的内容不好」。

   > **口径本身的教训**：「三取最小」是为了与屏5 `avgQuality` 同源、让「41→89」两端可比
   > 而选的（决策 K），那个目的是**展示**。把同一个口径直接拿来当**通过/不通过的闸门**，
   > 就把「任一指标不可靠」放大成了「补库失败」——这两件事该分开。

   **修法**（按收益排）：① `gap_clusters` 加三列存明细，屏5 展开行显示，让人看得见是哪一项拖的；
   ② 任一指标为 0/未评时走 `verifyInconclusive` 而非 `verifyFail`（**不打复发标**，
   文案改「本次未能评出可信分数」）——与本波「基础设施故障不该说成业务结论」的推理同构，
   当时只是没想到 `ctxprec=0` 属于「分数不可信」而非「分数很低」；
   ③ **缺口 23 应当提级**：它此前归类为「慢慢修」，现在卡死了 B2b 的核心闭环。
