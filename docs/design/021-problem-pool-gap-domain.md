...
> **为什么写在这里**：这些原本记在 `.ship/tasks/eval-w4-b2b/review/concerns.md`，
> 而 `.ship/` 被 gitignore —— 新会话在 `main` 上**看不到它**，只会看到「B2b 已交付、CI 全绿」。
> M8 踩过同款坑（见 `002` 的分波交付进度表）。跨会话可见的进度与欠账，一律走已提交的 `docs/design/`。
>
> 已在交付内修掉的不再列（`terminal_at`/CAS 空测、`filled` 兜底扫描、upload 与 CAS 的顺序、
> 决策 J 的草稿出口、限频器泄漏、手动入池丢改写）——它们的经过见 §11.7 与 git 历史。

### 12.1 明知该做而**完全没做**的（只有一条，优先级最高）

**原型逐屏视觉比对从未完成。** CLAUDE.md 明确要求「写/改任何前端页面前，必须先读原型
对应屏并 1:1 还原」，而 B2b 全程没做到：QA 环境 `computer{screenshot}` 稳定 30s 超时、
antd Drawer 渲染在视口外，所有前端证据都是 **DOM 文本**。
⇒ **功能验过，界面是否 1:1 还原完全未知**。原型在 `RAGForge-m7a/RAG知识库问答系统设计/CodeCrushBot.dc.html`（§17.5）。

### 12.2 同一类 bug 的另一半（暂时不发作，但会）

**回验缺「代表问题可否独立检索」的守卫。**

§11.7 的根因（入池丢改写）已修，所以现在代表问题是可独立检索的。
但 rewrite **真的失败**时（缺口 23 的场景，`rewriteResolved=false`），
回验仍会拿一句带指代的原话去重放 ⇒ 检索召不回 ⇒ 低分 ⇒ **判「补库后仍低分」+ 打复发标**。
即：**把「题目不成立」说成了「补库失败」**——与已修的那个 bug 后果完全一样，只是触发条件更窄。

> 讽刺之处在于：评测臂**早就拦住了**这种问题（「进评测集」弹窗强制人先改写，
> tooltip 逐字写着「离线评测无对话上下文，指代原文永远答不对，会成为永久 0 分用例」）。
> **两条臂对同一个信号的处置至今仍不一致。**

**修法**：`verifyCluster` 开头判代表问题是否可独立检索，否 ⇒ 走 `verifyInconclusive`
（回 `pending`、**不打复发标**、文案说明「代表问题需先改写才能回验」）。
与本波「基础设施故障不该说成业务结论」的推理同构。

**顺带**：回验只落一个合成分（`min` 三项），三项明细一项都不存 ⇒ 低分时无法归因。
§11.7 那次能查清全靠直接翻 ClickHouse，运营没有这个能力。建议 `gap_clusters` 加三列存明细。

### 12.3 聚类阈值与 rewrite 的输出风格

实测：两条同主题问题的改写结果余弦相似度 **0.7927**，阈值 0.85，**差 0.057 没聚上**。

原因不是模型不行，是 **rewrite 的输出风格不稳定**——同一批里有的产出完整问句
（「团队核心骨干员工提出离职，管理者如何运用人力资源管理策略或激励理论进行员工保留？」），
有的产出名词短语（「核心员工离职挽留策略与人力资源管理」）。形态差异直接打在相似度上。
**改写反而把它们推得比原话更远**（原话句式是一致的）。

三条路，倾向 B：
- **A. 降阈值** 0.85 → 0.80 —— 最省事，但阈值是全局的，会连带把不该聚的也聚进来
- **B. 约束 rewrite 的输出格式**（在 prompt 里钉死「必须输出一个完整问句」）——
  形态统一了同主题自然更近，且**对下游检索本身也更好**：名词短语式查询与文档的匹配度不如问句
- **C. 接受现状**，靠屏5 既有的「合并簇」人工处理（原型 `:366` 本就承认「机器会聚错」）

### 12.4 性能（不急，但都是真实浪费）

| 项 | 浪费 | 修法 |
|---|---|---|
| `GapPromoteService` 每次 promote 重算目标集前 200 条既有用例的 embedding | 约 6k token/次**纯重复**（那 200 条是不变的） | 给 `eval_case_versions` 加 `question_embedding` 列（同 `gap_clusters.centroid` 存法）；进程内缓存无效——api/worker 已分进程，worker 侧永远是冷的 |
| `mustFind` 走全列 `select()`，每次拖 1024 维 centroid | 单次向导提交约 45 KB 白传（`submitFill` 走 3 遍、`draftFill` 3 遍、`verifyCluster` 2 遍） | 加一个显式列清单的 `findClusterMeta`，全列版本只留给真要 centroid 的路径 |

> 注：`gaps.repository.ts` 的 `findClusterByFillTargetDocument` 注释里逐字写了
> 「把 1024 维 centroid 拖出来纯属浪费」，然后紧挨着的所有新调用点全走了拖 centroid 的那条路。

### 12.5 验证覆盖的空白

- **`≥80 → 已回验✓` 已跑通**（§11.7），但**复发重开的正向路径仍只验了负半**
  （手动入池超阈值确实不触发重开）。正向需要向 ClickHouse 灌够匹配样本并等收集器 cron。
- **P2-4 的时序窗口未复现**：需要在写完 `documents.status` 之后、广播之前杀 worker。
  兜底扫描已实现，但没在真实的「广播丢失」场景下验证过。
- **review 阶段前端维度无独立复审**（该复审员中途被停止，后补了一轮，抓到 3 条实质问题已修）。

### 12.6 两条方法论教训（比上面的清单更值得读）

1. **诊断连错两次的共同点**（§11.7 详述）：从观测到的现象**直接跳到最近的已知缺陷**，
   没有先问「这个现象的上游是什么、那一段的数据长什么样」。
   真正解题的线头是「两条臂对同一个信号的处置自相矛盾」。
2. **桩把输入抹平到分不出对错时，它守的只是『没抛异常』**：
   `gaps.e2e.spec.ts` 的 embedding 桩对所有文本返回同一个向量，
   于是「聚类键用了哪段文本」在测试里恒真——聚类键漂移因此长期潜伏。
   同理，回验的单测都喂**构造的分数**，没有任何一层去问「这个分数本身可不可信」。
