...
draft——产品裁决、独立调查、peer diff 与执行演练已完成；实现与全量验证完成后升 current。
...
Rollout：先发布权威文档；实现并验证 v2；停止 worker；确认无 queued/running run；执行 0021；部署
v2 API/worker；观察 evaluation success/failure、faithfulness coverage 与 sentinel 泄漏断言。若要
