...
17. **`rewrite`/`intent` 节点被思考 token 拖垮 —— 慢的真正根因**（QA 直接实测，**已排独立波次**）——
    ...
    **为什么单独成波、不在 QA 修复里顺手做**：它要动 **models adapter + 四个节点的 params**——
    正是 chat 关键路径（本波的 Global Constraints 明令「线上问答行为逐字节不变」）。
    改 thinking 开关会改变模型输出分布，属**行为性变更**，需要自己的 design→dev 闭环与回归证据，
    不能作为「评测超时修复」的搭车项。已回写 `002` 附录 A 作为后续波次。
