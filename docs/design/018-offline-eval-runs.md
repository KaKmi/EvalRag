...
2. **超时口径偏离原型 §6** —— 原型说超时「记 0 分」，本设计记 **NULL + `verdict=timeout`**。理由：同段原型自己规定裁判失败「不记 0 分，不拉低均值」——两条规则对同一张记分卡的口径**自相矛盾**。统一为「没评出来就不进 avg」，报告用 `verdict` 列 + 覆盖率显性表达超时，信息不丢失且均值不被污染。
3. **每题重复 `repeat_count` 未做** —— 原型 §6 有「每题重复 1-5」控件，但逐用例表是一行一用例（§7），**多次重复如何聚合到一行原型未定义**（取均值？取最差？分行？）。半实现会产出歧义数据。W2a 行为 = 重复 1 次（原型默认值）。W2b 与「范围（仅上次低分/按标签）」一并做时定聚合口径再加列。
